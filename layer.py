...
class CBAM(nn.Module):
    def __init__(self, nch_in, nch_hidden, kernel_size=7, stride=1, padding=3, relu=0.0, pool_types=['avg', 'max']):
        super().__init__()

        self.att_ch = ChannelAttention(nch_in=nch_in, nch_hidden=nch_hidden, pool_types=pool_types, relu=relu)
        self.att_sp = SpatialAttention(nch_in=nch_in, nch_hidden=nch_hidden, pool_types=pool_types,
                                       kernel_size=kernel_size, stride=stride, padding=padding, relu=relu)

    def forward(self, x):
        return self.att_sp(self.att_ch(x))


class ChannelAttention(nn.Module):
    def __init__(self, nch_in, nch_hidden, pool_types=['avg', 'max'], relu=0.0):
        super().__init__()

        layers = []
        layers += [Flatten()]
        layers += [Linear(nch_in=len(pool_types) * nch_in, nch_out=nch_hidden)]
        layers += [ReLU(relu)]
        layers += [Linear(nch_in=nch_hidden, nch_out=nch_in)]
    ...
    def forward(self, x):
        ...
        for pool_type in self.pool_types:
            if pool_type == 'avg':
                pool = F.avg_pool2d(x, kernel_size=(x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
                cat = torch.cat([cat, pool], dim=1)

            elif pool_type == 'max':
                pool = F.max_pool2d(x, kernel_size=(x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
                cat = torch.cat([cat, pool], dim=1)

        conv = self.conv(cat)
        att_ch = torch.sigmoid(conv).unsqueeze(2).unsqueeze(3)

        return x * att_ch

class SpatialAttention(nn.Module):
    def __init__(self, nch_in, nch_hidden, kernel_size=7, stride=1, padding=3, pool_types=['avg', 'max'], relu=0.0):
        super().__init__()

        layers = []
        layers += [Conv2d(nch_in=len(pool_types) * nch_in, nch_out=nch_hidden, kernel_size=kernel_size, stride=stride, padding=padding)]
        layers += [ReLU(relu)]
        layers += [Conv2d(nch_in=nch_hidden, nch_out=nch_in, kernel_size=kernel_size, stride=stride, padding=padding)]
    ...
    def forward(self, x):
        ...
        for pool_type in self.pool_types:
            if pool_type == 'avg':
                pool = F.avg_pool3d(x.unsqueeze(1), kernel_size=(x.size(1), 1, 1), stride=(x.size(1), 1, 1)).squeeze(1)
                cat = torch.cat([cat, pool], dim=1)

            elif pool_type == 'max':
                pool = F.max_pool3d(x.unsqueeze(1), kernel_size=(x.size(1), 1, 1), stride=(x.size(1), 1, 1)).squeeze(1)
                cat = torch.cat([cat, pool], dim=1)

        conv = self.conv(cat)
        att_sp = torch.sigmoid(conv)
